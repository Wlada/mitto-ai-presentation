...
[ Try the demo feature -> ]
   ...
   51 feature tests      100% backend coverage
   4 feature e2e         94% frontend coverage

   Part of a 70-test codebase — 93% backend / 84% frontend coverage app-wide.
...
The headline cards count only what the agent produced for **this feature**
(the Q&A page and its backend). The smaller line gives app-wide context so the
audience knows the numbers aren't cherry-picked from a tiny scope. All numbers
come from
...
- *"51 unit and integration tests for the feature, 100% backend coverage on its code, 94% frontend, and 4 Playwright e2e flows. These are the numbers for the feature you just saw built — and they sit inside a 70-test codebase that's 93% / 84% covered overall."*
